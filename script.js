(() => {
  ...
  const MODELS = [
    ...
    {
      theme:'midnight', model:'03 · Lunar Chrome', name:'Renata', date:'2027-05-15T21:30:00', dateLabel:'15 · 05 · 2027',
    ...
    {
      theme:'pearl', model:'05 · Secret Garden', name:'Martina', date:'2027-08-21T21:00:00', dateLabel:'21 · 08 · 2027',
  ...
  function galleryFigures(m){
    return m.gallery.map((src,j)=>`<figure class="reveal" style="--delay:${(j%3)*85}ms"><img data-lightbox-img src="${src}" alt="Galería de ${escapeHtml(m.name)} · foto ${j+1}" loading="lazy" decoding="async" referrerpolicy="no-referrer"></figure>`).join('');
  }

  function studioFooter(m){
    return `<footer class="studio-footer"><div><b class="studio-brand">ARTEDECO.STUDIO</b><span>Invitaciones digitales personalizadas · Bodas · XV · Eventos</span></div><div><b>¿TE GUSTÓ ESTE DEMO?</b><span>Fotos, música, colores, textos y secciones se adaptan a cada fiesta.</span><a class="studio-social" href="https://www.instagram.com/artedecostudio/" target="_blank" rel="noopener">${icon.instagram}<span>@artedecostudio</span></a></div></footer><audio class="invite-audio" src="${m.music}" preload="metadata" loop></audio>`;
  }

  function templateClassic(m, i){
    const figs = galleryFigures(m);
    return `<article class="invite theme-${m.theme}" data-index="${i}" data-name="${escapeHtml(m.name)}" data-date="${m.date}" data-location="${escapeHtml(m.map)}">
      ...
      <section class="band count-band"><p class="reveal">Falta cada vez menos</p><div class="countdown reveal" style="--delay:100ms" data-countdown></div></section>

      <section class="section event-section"><div class="event-card reveal"><div class="event-photo" style="background-image:url('${m.eventPhoto}')"></div><div class="event-copy"><div class="event-icon">${icon.party}</div><p class="eyebrow">La fiesta</p><h2>${escapeHtml(m.venue)}</h2><p>${escapeHtml(m.venueDetail)}<br>Una noche de música, fotos, sorpresas y mucha pista.</p><div class="event-actions"><button class="pill filled" data-map="${escapeHtml(m.map)}">Cómo llegar</button><button class="pill" data-calendar>Agendar</button></div></div></div></section>

      <section class="section gallery"><div class="wrap"><p class="eyebrow reveal">Un poquito de mí</p><h2 class="section-title reveal" style="--delay:70ms">Fotos que también cuentan la historia</h2><p class="section-copy reveal" style="--delay:130ms">La galería se puede personalizar con sesión de fotos, recuerdos, backstage o imágenes de la familia.</p><div class="gallery-grid">${figs}</div></div></section>

      <section class="section extras"><div class="wrap"><p class="eyebrow reveal">Detalles de la noche</p><h2 class="section-title reveal" style="--delay:70ms">Todo lo que tus invitados necesitan saber</h2><div class="extra-grid"><article class="extra-card dress reveal" style="--delay:100ms"><div class="extra-icon">${icon.dress}</div><h3>Dress code</h3><p>${escapeHtml(m.dress)}</p></article><article class="extra-card instagram reveal" style="--delay:160ms"><div class="extra-icon">${icon.instagram}</div><h3>Instagram</h3><p>${escapeHtml(m.instagram)}<br>#${escapeHtml(m.name)}XV</p></article><article class="extra-card transport reveal" style="--delay:220ms"><div class="extra-icon">${icon.car}</div><h3>Traslados</h3><p>${escapeHtml(m.transport)}</p></article><article class="extra-card schedule reveal" style="--delay:280ms"><div class="extra-icon">${icon.clock}</div><h3>Horario</h3><p>Recepción puntual y pista hasta el cierre.</p></article></div></div></section>

      <section class="band gift-section"><div class="wrap gift-wrap"><div class="gift-anim reveal" aria-hidden="true"><svg viewBox="0 0 140 120"><g><rect x="34" y="61" width="72" height="42" rx="3"/><path d="M70 61v42"/></g><g class="gift-spark"><path d="M68 53V38M60 46h16"/><circle cx="88" cy="42" r="2.2"/></g><g class="gift-lid"><rect x="28" y="43" width="84" height="14" rx="3"/><path d="M70 43v14"/><path d="M69 43C58 39 50 34 52 27c2-6 9-4 14 1 3 4 3 9 3 15Z"/><path d="M71 43c11-4 19-9 17-16-2-6-9-4-14 1-3 4-3 9-3 15Z"/></g></svg></div><p class="eyebrow reveal" style="color:inherit">Regalos</p><h2 class="section-title reveal" style="--delay:80ms">Tu presencia es lo más importante</h2><p class="section-copy reveal" style="--delay:150ms">Si querés hacerme un regalo, podés ver acá los datos elegidos para esta invitación.</p><button class="pill light reveal" style="--delay:230ms" data-gift>Ver datos de regalo</button></div></section>

      <section class="section playlist"><div class="wrap"><div class="vinyl reveal" aria-hidden="true"></div><p class="eyebrow reveal" style="--delay:70ms">La playlist también la armamos juntos</p><h2 class="section-title reveal" style="--delay:130ms">¿Qué canción no puede faltar?</h2><p class="section-copy reveal" style="--delay:190ms">Mandame ese tema que te hace salir a bailar sí o sí.</p><button class="pill filled reveal" style="--delay:260ms" data-song>Proponer canción</button></div></section>

      <section class="section useful"><div class="wrap"><p class="eyebrow reveal">Info útil</p><h2 class="section-title reveal" style="--delay:70ms">Para tener todo a mano</h2><div class="useful-grid"><article class="useful-card reveal" style="--delay:100ms">${icon.calendar}<h3>Fecha</h3><p>${escapeHtml(m.dateLabel)}<br>${escapeHtml(m.venueDetail.split('·')[0].trim())}</p></article><article class="useful-card reveal" style="--delay:170ms">${icon.pin}<h3>Ubicación</h3><p>${escapeHtml(m.venue)}<br>Montevideo</p></article><article class="useful-card reveal" style="--delay:240ms">${icon.instagram}<h3>Compartí</h3><p>Etiquetá ${escapeHtml(m.instagram)}<br>y usá #${escapeHtml(m.name)}XV</p></article></div></div></section>

      <section class="section rsvp"><div class="rsvp-card reveal"><p class="eyebrow">Confirmación</p><h2 class="section-title">¿Venís a celebrar conmigo?</h2><p class="section-copy">Confirmá asistencia y contame si tenés alguna necesidad alimentaria o dato que debamos tener en cuenta.</p><button class="pill filled" data-rsvp="${escapeHtml(m.name)}">Confirmar asistencia</button></div></section>
  ...
  function templateMidnight(m, i){
    const figs = galleryFigures(m);
    return `<article class="invite theme-midnight model-03" data-index="${i}" data-name="${escapeHtml(m.name)}" data-date="${m.date}" data-location="${escapeHtml(m.map)}">
      <section class="section m3-hero">
        <div class="m3-gridlines" aria-hidden="true"></div>
        <div class="m3-number reveal">15</div>
        <div class="m3-copy">
          <p class="m3-overline reveal">LUNAR CHROME · XV EXPERIENCE</p>
          <h1 class="reveal" style="--delay:90ms">${escapeHtml(m.name)}</h1>
          <p class="m3-date reveal" style="--delay:150ms">${escapeHtml(m.dateLabel)}</p>
          <p class="m3-quote reveal" style="--delay:220ms">“${escapeHtml(m.quote)}”</p>
          <button class="m3-scroll reveal" style="--delay:300ms" data-next-section>ENTER THE NIGHT ↓</button>
        </div>
        <div class="m3-portrait reveal" style="--delay:120ms"><div class="m3-photo" style="background-image:url('${m.hero}')"></div><span>RENATA / 15 / 2027</span></div>
      </section>

      <section class="m3-count"><div class="m3-count-label reveal">COUNTDOWN TO MIDNIGHT</div><div class="countdown reveal" style="--delay:80ms" data-countdown></div></section>

      <section class="section m3-manifesto"><div class="m3-manifesto-inner"><span class="m3-vertical">A NIGHT TO REMEMBER</span><div><p class="eyebrow reveal">Mi noche</p><h2 class="reveal" style="--delay:70ms">Una fiesta con estética nocturna, reflejos metálicos y mucha pista.</h2><p class="reveal" style="--delay:130ms">Este modelo cambia la lógica clásica por una invitación tipo editorial de moda: composición asimétrica, grandes números, líneas finas y una atmósfera de “after dark”.</p></div></div></section>

      <section class="section m3-event"><div class="m3-event-photo reveal" style="background-image:url('${m.eventPhoto}')"><div class="m3-event-tag">15 MAY · 21:30</div></div><div class="m3-event-panel reveal" style="--delay:100ms"><p class="eyebrow">THE VENUE</p><h2>${escapeHtml(m.venue)}</h2><p>${escapeHtml(m.venueDetail)}<br>${escapeHtml(m.dress)}</p><div class="event-actions"><button class="pill filled" data-map="${escapeHtml(m.map)}">Cómo llegar</button><button class="pill" data-calendar>Agendar</button></div></div></section>

      <section class="section m3-film"><div class="wrap"><div class="m3-film-head"><div><p class="eyebrow reveal">EDITORIAL / 01—06</p><h2 class="section-title reveal" style="--delay:60ms">Six frames of me</h2></div><p class="reveal" style="--delay:120ms">Sesión fotográfica en formato editorial: grande, limpia y protagonista.</p></div><div class="m3-filmstrip">${figs}</div></div></section>

      <section class="section m3-dashboard"><div class="wrap"><p class="eyebrow reveal">NIGHT FILE</p><h2 class="section-title reveal">Detalles, sin ruido</h2><div class="m3-dashboard-grid"><article class="reveal">${icon.dress}<b>DRESS</b><span>${escapeHtml(m.dress)}</span></article><article class="reveal" style="--delay:70ms">${icon.instagram}<b>SOCIAL</b><span>${escapeHtml(m.instagram)} · #${escapeHtml(m.name)}XV</span></article><article class="reveal" style="--delay:140ms">${icon.car}<b>ARRIVAL</b><span>${escapeHtml(m.transport)}</span></article><article class="reveal" style="--delay:210ms">${icon.clock}<b>DOORS</b><span>21:30 · puntual</span></article></div></div></section>

      <section class="section m3-sound"><div class="m3-disc reveal"><span>XV</span></div><div class="m3-sound-copy"><p class="eyebrow reveal">SOUNDTRACK</p><h2 class="section-title reveal">La música también habla de vos</h2><p class="section-copy reveal">Sumá tu canción a una playlist pensada para empezar elegante y terminar arriba.</p><button class="pill filled reveal" data-song>Sumar canción</button></div></section>

      <section class="section m3-gift-rsvp"><div class="m3-silver-card reveal"><p class="eyebrow">GIFT</p><h3>Tu presencia es el regalo principal.</h3><p>Si querés sumar algo más, acá van los datos elegidos por la familia.</p><button class="pill" data-gift>Ver regalo</button></div><div class="m3-silver-card m3-rsvp-card reveal" style="--delay:90ms"><p class="eyebrow">RSVP</p><h3>Confirmá tu lugar en la noche.</h3><p>Asistencia, alimentación y mensaje personal.</p><button class="pill filled" data-rsvp="${escapeHtml(m.name)}">Confirmar</button></div></section>

      <section class="m3-closing"><span>15</span><div><p>RENATA</p><b>${escapeHtml(m.dateLabel)}</b></div></section>
      ${studioFooter(m)}
    </article>`;
  }

  function templateCherry(m, i){
    const figs = galleryFigures(m);
    return `<article class="invite theme-cherry model-04" data-index="${i}" data-name="${escapeHtml(m.name)}" data-date="${m.date}" data-location="${escapeHtml(m.map)}">
      <section class="section m4-hero">
        <div class="m4-checkers" aria-hidden="true"></div><div class="m4-glow" aria-hidden="true"></div>
        <div class="m4-sticker m4-sticker-a">XV!</div><div class="m4-sticker m4-sticker-b">DANCE</div>
        <div class="m4-hero-card reveal"><img src="${m.hero}" alt="${escapeHtml(m.name)}" referrerpolicy="no-referrer"><span>05.06.27</span></div>
        <div class="m4-hero-copy"><p class="m4-kicker reveal">CHERRY POP CLUB</p><h1 class="reveal" style="--delay:80ms">${escapeHtml(m.name)}</h1><p class="m4-xv reveal" style="--delay:140ms">FIFTEEN</p><p class="reveal" style="--delay:200ms">${escapeHtml(m.quote)}</p><button class="m4-button reveal" style="--delay:270ms" data-next-section>LET'S GO ↓</button></div>
      </section>

      <section class="m4-marquee" aria-hidden="true"><div>LET'S DANCE · XV PARTY · LET'S DANCE · XV PARTY · LET'S DANCE · XV PARTY ·&nbsp;</div></section>

      <section class="section m4-ticket-section"><div class="m4-ticket reveal"><div class="m4-ticket-left"><span>ADMIT ONE</span><b>XV</b><small>${escapeHtml(m.name)}</small></div><div class="m4-ticket-main"><p>THE PARTY</p><h2>${escapeHtml(m.venue)}</h2><h3>${escapeHtml(m.venueDetail)}</h3><div class="m4-ticket-actions"><button class="pill filled" data-map="${escapeHtml(m.map)}">Cómo llegar</button><button class="pill" data-calendar>Agendar</button></div></div><div class="m4-barcode" aria-hidden="true"></div></div></section>

      <section class="section m4-count-section"><div class="m4-disco reveal" aria-hidden="true"><span></span></div><div><p class="eyebrow reveal">COUNTDOWN</p><h2 class="section-title reveal">La pista abre en…</h2><div class="countdown reveal" data-countdown></div></div></section>

      <section class="section m4-polaroid-section"><div class="wrap"><p class="eyebrow reveal">CAMERA ROLL</p><h2 class="section-title reveal">Momentos, poses y cero solemnidad</h2><div class="m4-polaroids">${figs}</div></div></section>

      <section class="section m4-stickers-section"><div class="wrap"><p class="eyebrow reveal">PARTY INFO</p><h2 class="section-title reveal">Todo lo importante, en modo sticker</h2><div class="m4-info-bubbles"><article class="bubble b1 reveal">${icon.dress}<b>LOOK</b><span>${escapeHtml(m.dress)}</span></article><article class="bubble b2 reveal" style="--delay:70ms">${icon.instagram}<b>POST IT</b><span>${escapeHtml(m.instagram)}<br>#${escapeHtml(m.name)}XV</span></article><article class="bubble b3 reveal" style="--delay:140ms">${icon.car}<b>RIDE</b><span>${escapeHtml(m.transport)}</span></article><article class="bubble b4 reveal" style="--delay:210ms">${icon.clock}<b>START</b><span>22:00 SHARP</span></article></div></div></section>
      ...
      <section class="section m4-gift"><div class="m4-gift-copy"><p class="eyebrow reveal">GIFT DROP</p><h2 class="section-title reveal">Presencia primero. Regalo después.</h2><p class="section-copy reveal">Si querés hacerme un regalo, encontrás los datos acá.</p><button class="pill filled reveal" data-gift>Ver datos</button></div><div class="m4-gift-box reveal" aria-hidden="true"><div class="lid"></div><div class="body">XV</div></div></section>

      <section class="section m4-rsvp"><div class="m4-rsvp-card reveal"><span class="m4-rsvp-star">★</span><p class="eyebrow">RSVP</p><h2>¿Te sumás al club?</h2><p>Confirmá y guardamos tu lugar en la pista.</p><button class="m4-button dark" data-rsvp="${escapeHtml(m.name)}">CONFIRMAR</button></div></section>
      <section class="m4-closing"><div>SEE YOU ON THE DANCE FLOOR</div><h2>${escapeHtml(m.name)}</h2><p>${escapeHtml(m.dateLabel)}</p></section>
      ${studioFooter(m)}
    </article>`;
  }

  function templatePearl(m, i){
    const figs = galleryFigures(m);
    return `<article class="invite theme-pearl model-05" data-index="${i}" data-name="${escapeHtml(m.name)}" data-date="${m.date}" data-location="${escapeHtml(m.map)}">
      <section class="section m5-hero">
        <div class="m5-flower f1" aria-hidden="true">✿</div><div class="m5-flower f2" aria-hidden="true">❀</div><div class="m5-flower f3" aria-hidden="true">✾</div>
        <div class="m5-big15" aria-hidden="true">15</div>
        <div class="m5-arch reveal"><div class="m5-arch-photo" style="background-image:url('${m.hero}')"></div></div>
        <div class="m5-hero-copy"><p class="m5-kicker reveal">SECRET GARDEN · MIS XV</p><h1 class="reveal" style="--delay:80ms">${escapeHtml(m.name)}</h1><p class="m5-date reveal" style="--delay:140ms">${escapeHtml(m.dateLabel)}</p><p class="m5-quote reveal" style="--delay:200ms">${escapeHtml(m.quote)}</p><button class="m5-scroll reveal" style="--delay:280ms" data-next-section>descubrir la invitación ↓</button></div>
      </section>

      <section class="section m5-pearl-count"><div class="wrap"><p class="eyebrow reveal">La espera también es parte de la magia</p><div class="m5-count-shell reveal" style="--delay:80ms"><div class="countdown" data-countdown></div></div></div></section>

      <section class="section m5-letter"><div class="m5-letter-paper reveal"><span class="m5-wax">15</span><p class="eyebrow">UNA NOCHE PARA RECORDAR</p><h2>Quiero compartir con vos uno de esos momentos que quedan para siempre.</h2><p>Una invitación de inspiración jardín, con formas orgánicas, lavanda, perla y detalles suaves. Más romántica, pero sin sentirse infantil.</p></div></section>

      <section class="section m5-venue"><div class="m5-venue-card reveal"><div class="m5-venue-photo" style="background-image:url('${m.eventPhoto}')"></div><div class="m5-venue-copy"><span class="m5-mini-flower">❦</span><p class="eyebrow">LA CELEBRACIÓN</p><h2>${escapeHtml(m.venue)}</h2><p>${escapeHtml(m.venueDetail)}<br>${escapeHtml(m.dress)}</p><div class="event-actions"><button class="pill filled" data-map="${escapeHtml(m.map)}">Cómo llegar</button><button class="pill" data-calendar>Agendar</button></div></div></div></section>

      <section class="section m5-scrap"><div class="wrap"><p class="eyebrow reveal">MY LITTLE ALBUM</p><h2 class="section-title reveal">Un álbum que se siente hecho a mano</h2><p class="section-copy reveal">Las fotos aparecen como recuerdos impresos: distintos tamaños, pequeñas rotaciones y mucho aire.</p><div class="m5-scrapbook">${figs}</div></div></section>

      <section class="section m5-ribbon-info"><div class="wrap"><p class="eyebrow reveal">ANTES DE VENIR</p><h2 class="section-title reveal">Cuatro detalles, cuatro cintas</h2><div class="m5-ribbons"><article class="reveal"><span>01</span><b>Dress code</b><p>${escapeHtml(m.dress)}</p></article><article class="reveal" style="--delay:70ms"><span>02</span><b>Compartí</b><p>${escapeHtml(m.instagram)}<br>#${escapeHtml(m.name)}XV</p></article><article class="reveal" style="--delay:140ms"><span>03</span><b>Traslados</b><p>${escapeHtml(m.transport)}</p></article><article class="reveal" style="--delay:210ms"><span>04</span><b>Horario</b><p>21:00 · recepción y bienvenida</p></article></div></div></section>

      <section class="section m5-gift-song"><div class="m5-garden-card reveal"><span class="m5-card-icon">✧</span><p class="eyebrow">REGALO</p><h3>Tu presencia ya es parte del recuerdo.</h3><p>Si querés sumar un regalo, encontrás los datos acá.</p><button class="pill" data-gift>Ver datos</button></div><div class="m5-garden-card reveal" style="--delay:90ms"><span class="m5-card-icon">♪</span><p class="eyebrow">PLAYLIST</p><h3>Una canción para mi noche.</h3><p>Dejame ese tema que para vos no puede faltar.</p><button class="pill filled" data-song>Proponer canción</button></div></section>

      <section class="section m5-rsvp"><div class="m5-rsvp-card reveal"><div class="m5-rsvp-ornament">❦</div><p class="eyebrow">CONFIRMACIÓN</p><h2>¿Vas a estar conmigo?</h2><p>Confirmá asistencia y contame cualquier detalle que necesitemos saber.</p><button class="pill filled" data-rsvp="${escapeHtml(m.name)}">Confirmar asistencia</button></div></section>
      <section class="m5-closing"><p>con cariño,</p><h2>${escapeHtml(m.name)}</h2><span>XV · ${escapeHtml(m.dateLabel)}</span></section>
      ${studioFooter(m)}
    </article>`;
  }

  function template(m, i){
    if(i===2) return templateMidnight(m,i);
    if(i===3) return templateCherry(m,i);
    if(i===4) return templatePearl(m,i);
    return templateClassic(m,i);
  }
